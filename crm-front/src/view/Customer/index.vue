<!-- 客户管理 -->
<template>
  <component :is="list.List" title="客户管理">
...
<script setup name="Customer">
  ...
  import { delCustomer, batchDelCustomer } from '@/api/customer.js';
  ...
  const column = ref([
    ...
    // { show: true, prop: 'activityName', label: '所属活动', width: '100' },
  ...
  const getTableDataParam = () => ({
    apiChain: ['customer', 'getCustomers'],
    param: {
  ...
  // 创建组件映射对象
  const componentMap = {
    Detail: markRaw(CustomerDetail),
    AddForm: markRaw(CustomerForm),
    EditForm: markRaw(CustomerForm)
  };
